...
year_min = search_criteria[0]
engine_volume_min = search_criteria[1]
price_max = search_criteria[2]

filtered = [[car_data[name][4], name, car_data[name][0], car_data[name][1], car_data[name][2], car_data[name][3]] for name in car_data
            if car_data[name][1] >= year_min
            and car_data[name][2] >= engine_volume_min
            and car_data[name][4] <= price_max]

filtered_sorted = sorted(filtered)
print("Автомобілі, які відповідають критеріям пошуку:")
for car in filtered_sorted[:5]:
    price, name, color, year, engine_volume, car_type = car
    print(f"{name}: {color}, {year} рік, {engine_volume} л., {car_type}, ціна: {price}$")
