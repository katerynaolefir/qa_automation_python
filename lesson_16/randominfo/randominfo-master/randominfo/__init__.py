...
def get_id(length = 6, seq_number = None, step = 1, prefix = None, postfix = None):
	...
	if seq_number is None:
		...
		if not isinstance(seq_number, int) and not isinstance(step, int):
...
def get_first_name(gender = None):
	with open(BASE_FOLDER / 'data.csv') as f:
		first_name_file = list(csv.reader(f))
...
def get_last_name():
	with open(BASE_FOLDER / 'data.csv') as f:
		last_name_file = list(csv.reader(f))
	filtered_data = []
...
def get_gender(first_name):
	...
	gender = ""
	for data in first_name_file:
		if data[0] != '' and data[0].lower() == first_name.lower():
			gender = data[2]
			break
	return gender
...
def get_country(first_name = None):
	with open(BASE_FOLDER / 'data.csv') as f:
		country_file = list(csv.reader(f))

	headers = country_file[0]
	body = country_file[1:]

	if "Country" not in headers:
		return "India"
	csv_as_dict = [dict(zip(headers, row)) for row in body]
	if first_name is not None:
		for row in csv_as_dict:
			if row.get("Name") == first_name:
...
def get_alphabetic_profile_img(char, file_path, img_name, char_color = None, bg_color = None):
	...
	if all((c in chars) for c in img_name):
		if access(dirname(file_path), W_OK):
			...
			font = ImageFont.truetype(BASE_FOLDER / "Candara.ttf", 280)
	...
	if all((c in chars) for c in img_name):
		if access(dirname(file_path), W_OK):
			if gender is None:
				...
				raise ValueError("Invalid gender. It must be male or female.")
...
date_start_range = datetime(1970, 1, 1, 0, 0, 0, 0, pytz.UTC)
date_end_range = datetime.today()
...
def get_date(tstamp = None, _format = "%d %b, %Y"):
	if tstamp is None:
		start_ts = date_start_range.timestamp()
		end_ts = datetime.timestamp(date_end_range)
		tstamp = randrange(int(start_ts), int(end_ts))
	else:
		if not isinstance(tstamp, int):
			raise ValueError("Timestamp must be an integer.")
	return datetime.fromtimestamp(tstamp, pytz.UTC).strftime(_format)
...
def get_hobbies():
	with open(BASE_FOLDER / 'data.csv') as f:
		hobbies_file = list(csv.reader(f))
	all_hobbies = []
